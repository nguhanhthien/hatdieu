...
class CrudRepository
{
    ...
    public function settingForm()
    {
        ...
        $form->add('description','Description', 'text')->rule('required|min:20');
        $form->submit('Lưu cài đặt');

        $form->saved(function () use ($form) {
            $form->message("Cài đặt đã được lưu");
    ...
    {
        return $table = with(new Setting())->getTable();
    }
     ...
     * @return mixed
     */
    public function settingsGrid()
    {
       // $form = \DataForm::source(new Setting);
       //or find a record to update some value
       $form = \DataForm::source(Setting::find(1));
       //add fields to the form
       $form->add('title','Title', 'text')->rule('required');
       $form->add('keyword','Keyword', 'text')->rule('required'); //validation
       $form->add('description','Description', 'text')->rule('required'); //validation
       ...
       $form->saved(function() use ($form)
       {
            ...
            $form->link("/settings/edit","Lưu cài đặt");
       });
       return $form;
    }
    ...
    public function catFilter()
    {
        ...
        $filter->add('cat', 'Tên danh mục', 'text');
        $filter->submit('Tìm');
        $filter->link('/backend/category/edit', "Tạo mới",'BL',['class' => 'btn btn-success pull-right']);
        $filter->build();
        return $filter;
    }
    ...
    public function catGrid()
    {
        if (request()->segment(2) === 'subcategory') {
            ...
            $grid->attributes(array("class" => "table table-striped custom"));
            ...
            $grid->attributes(array("class" => "table table-striped custom"));
            $grid->add('cat_id', 'ID', true)->style("width:100px");
            $grid->add('cat', 'Tên danh dục');
            $grid->add('slug', 'Đường dẫn tĩnh');
            $grid->add('m_img', 'Ảnh đại diện')->cell( function( $value, $row) {
                return ($value != '') ? "<img src=".$value." >" : "No image";
           });

            $grid->edit('/backend/category/edit','<i class="fa fa-cog"></i>')->style("width:151px;text-align:center");
        }
    ...
    public function catEdit()
    {
        ...
        $edit->attributes(array("class" => "form-custom"));
        $edit->set('parent_id', 0);
        $edit->add('cat', 'Tên danh mục', 'text');
        $edit->add('slug','Đường dẫn tĩnh', 'text');
        $edit->add('keywords','Keywords', 'text');
        $edit->add('description','Description', 'text');
        $edit->add('m_img','Ảnh đại diện', 'image')->move('images/')->fit(240, 160)->preview(120,80);
        $edit->link('/backend/category', "Trở lại", "TR",['class' => 'btn btn-success']);
        return $edit;
    }
